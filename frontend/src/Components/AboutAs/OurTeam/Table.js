...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await withLoader(() => getData(
      `/admin/about-us/our-team?page=${onPageChange}&per_page=${perPage}&term=${search}`
    ));
  ...
  useEffect(() => {
    getDataAll();
  }, [changeStatus, perPage, reset, show, show1, onPageChange, search]);

  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/admin/about-us/our-team/${id}`);
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/admin/about-us/our-team/${recordToDeleteId}`
      );
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"Our Team"} link={"/admin/about-us/social-media"} />
                        ...
                        <Table striped bordered hover responsive center>
                          <thead>
                            <tr className="">
                              ...
                              {/* {visible.col7 && (
                              <th className="tax-name">Logo Two</th>
                            )} */}
                              {visible.col3 && (
                              ...
                              data.data.data.map((d, index) => {
                                ...
                                return (
                                  <tr className="" key={index}>
                                    ...
                                    {visible.col5 && <td>{d?.designation}</td>}
                                    {visible.col6 && (
                                      <td>
                                        {d?.image && (
                                          <img
