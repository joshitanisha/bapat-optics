...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await getData(
      `/admin/masters/food-add-on-category?page=${onPageChange}&per_page=${perPage}&term=${encodeURIComponent(search)`
    );
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(
      `/admin/masters/food-add-on-category/${id}`
    );
  ...
  const ChangePupularStatus = async (id) => {
    const response = await editStatusData(
      `/admin/masters/food-add-on-category/popular/${id}`
    );
    setChangeStatus(response);
  };
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/admin/masters/food-add-on-category/${recordToDeleteId}`
      );
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"Food Add On Category"} link={"/masters/category"} />
                      ...
                      <div className="d-flex">
                        {user?.role_id === RoleId.Vendor && (
                          <div className="add me-3">
                            {isAllow.includes(IDS.FoodAddOnCategory.Add) ? (
                              <Link
