...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await getData(
      `/admin/masters/f-child-category?page=${onPageChange}&per_page=${perPage}
      &term=${encodeURIComponent(search)}&f_category_id=${searchCategory?.value || ""
      }&f_sub_category_id=${searchSubCategory?.value || ""}`
    );
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/admin/masters/f-child-category/${id}`);
  ...
  const DeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/admin/store/f-child-category/${recordToDeleteId}`
      );
  ...
  const BulkUpload = async (e) => {
    try {
      ...
      const response = await postData(
        "/admin/store/f-child-category/bulk",
        formData,
        {}
      );
      if (response?.success) {
  ...
  const Sample = async (e) => {
    try {
      await getDownloadDataExcel("/admin/store/f-child-category/sample", {}, "state");
    } catch (error) {
      console.error("Error uploading file:", error);
    }
  };
  ...
  const GetAllCategory = async () => {
    const response = await getData("/common/masters/all-f-category");
    if (response?.success) {
      setCategories(await Select2Data(response?.data, "f_category_id"));
    }
  };

  const GetAllSubCategory = async (id) => {
    const response = await getData(`/common/masters/all-f-sub-category/${id}`);
    if (response?.success) {
      setSubCategories(await Select2Data(response?.data, "f_sub_category_id"));
    }
  };
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"Child Category"} link={"/admin/masters/f-child-category"} />
        <div className="">
                    ...
                    <div className="row">
                      <div className="col-xxl-2 col-xl-2 col-lg-4 col-md-6 col-sm-6 col-12">
                        <div className="add me-3">
                          {isAllow.includes(IDS.FoodChildCategory.Add) ? (
                            <Link
                              // to="/admin/masters/f-child-category/add"
                              type="button"
                            ...
                            <div className="col-xxl-3 col-xl-4 col-lg-4 col-md-6  col-sm-6 col-12 mb-2">
                              <div className="num">
                                ...
                                <input
                                  ...
                                  id=""
                                  placeholder="Search Food Child Category"
                                  value={search}
                                  onChange={(e) => {
                            ...
                            {data?.data?.data?.map((d, index) => {
                              ...
                              return (
                                <tr className="" key={index}>
                                  ...
                                  {visible.col2 && <td>{d?.f_category?.name}</td>}
                                  {visible.col3 && (
                                    <td>{d?.f_sub_category?.name}</td>
                                  )}
                                  {visible.col4 && <td className="width_dertails_name_div">{d?.name}</td>}{" "}
                                  {visible.col7 && (
                                    <td>
                                      {d?.image && (
                                        <img
                                  ...
                                  {visible.col6 && (
                                    <td>
                                      <div className="d-flex">
                                        {isAllow.includes(
                                          IDS.ProductChildCategory.Edit
                                        ) ? (
                                          <Button
                                            // to={`/admin/masters/f-child-category/edit/${d?.id}`}
                                        ...
                                        {isAllow.includes(
                                          IDS.ProductChildCategory.Delete
                                        ) ? (
                                          <button
                                            onClick={() => {
