...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await getData(
      `/seller/masters/s-gst?page=${currentPage}&per_page=${perPage}&term=${search}`
    );
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/seller/masters/s-gst/${id}`);
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/seller/masters/s-gst/${recordToDeleteId}`
      );
  ...
  const Sample = async (e) => {
    try {
      await getDownloadDataExcel("/seller/masters/s-gst/sample", {}, "S_GST");
    } catch (error) {
      console.error("Error uploading file:", error);
    }
  };
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"GST"} link={"/seller/masters/s-gst"} />
