...
const Tables = () => {
  ...
  const getDataAll = async () => {
    ...
    const response = await withLoader(() => getData(
      `/admin/masters/brand?page=${onPageChange}&per_page=${perPage}&term=${encodeURIComponent(search)}`
    ));
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/admin/masters/brand/${id}`);
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/admin/masters/brand/${recordToDeleteId}`
      );
  ...
  const Sample = async (e) => {
    try {
      await getDownloadDataExcel("/admin/masters/brand/sample", {}, "Category");
    } catch (error) {
      console.error("Error uploading file:", error);
    }
  };
  ...
  const ChangeCustomerStatus = async (id) => {
    const response = await editStatusData(
      `/admin/masters/brand/customer/${id}`
    );
    setChangeStatus(response);
  };
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"Brands"} link={"/admin/masters/brand"} />
