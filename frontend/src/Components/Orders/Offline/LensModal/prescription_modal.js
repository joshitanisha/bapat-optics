  ...
  useEffect(() => {
    ...
    for (const id of keys) {
      ...
      if (nvChanged) {
        ...
        const desiredADD = Number(curNV) - Number(curDV);
...
const Lense_prescription_modal = ({
  ...
  const GetAllPrescriptionvalue = async (name) => {
    try {
      

      const response = await getData(
        `/common/masters/prescription-value?name=${name}`,
      );
