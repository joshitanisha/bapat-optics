...
import { IDS, Select2Data, Loader, useLoader } from "../../../utils/common";
...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await withLoader(() =>
      getData(
        `/admin/setting/all-banner?page=${onPageChange}&per_page=${perPage}&term=${search}`
      )
    );
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/admin/setting/all-banner/${id}`);
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(
        `/admin/setting/all-banner/${recordToDeleteId}`
      );
  ...
  const Sample = async (e) => {
    try {
      await getDownloadDataExcel(
        "/admin/setting/all-banner/sample",
        {},
        "Category"
      );
    } catch (error) {
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"All banner"} link={"/admin/setting/all-banner"} />
                    ...
                    <div className="col-xxl-2 col-xl-2 col-lg-4 col-md-6 col-sm-6 col-12">
                      <div className="add me-3">
                        {isAllow.includes(IDS.Brand.Add) ? (
                          <Link
                            // to="/admin/setting/all-banner/add"
                            type="button"
                      ...
                      <Table striped bordered hover responsive center>
                        <thead>
                          <tr className="">
                            ...
                            {visible.col2 && (
                              <th className="tax-name">Banner Type </th>
                            )}{" "}
                            {visible.col2 && (
                              <th className="tax-name">Category Name </th>
                            )}{" "}
                            {visible.col5 && (
                              <th className="tax-name"> Image / Video </th>
                            )}
                            {visible.col3 && (
