...
const Tables = () => {
  ...
  const getDataAll = async () => {
    const response = await getData(
      `/admin/career/qualification?page=${onPageChange}&per_page=${perPage}&term=${encodeURIComponent(search)}`
    );
  ...
  const ChangeStatus = async (id) => {
    const response = await editStatusData(`/admin/career/qualification/${id}`);
  ...
  const handleDeleteRecord = async () => {
    ...
    if (recordToDeleteId) {
      const response = await deleteData(`/admin/career/qualification/${recordToDeleteId}`);
  ...
  const BulkUpload = async (e) => {
    try {
      ...
      const response = await postData("/admin/career/qualification/bulk", formData, {});
      if (response?.success) {
  ...
  const Sample = async (e) => {
    try {
      await getDownloadDataExcel("/admin/career/qualification/sample", {}, "Qualification");
    } catch (error) {
      console.error("Error uploading file:", error);
    }
  };
  ...
  return (
    <>
      <div className="main-advancedashboard">
        <Header title={"Qualification"} link={"/admin/career/qualification"} />
                    ...
                    <div className="col-xxl-2 col-xl-2 col-lg-4 col-md-6 col-sm-6 col-12">
                      <div className="add me-3">
                        {isAllow.includes(IDS.Country.Add) ? (
                          <Link
                            // to="/admin/career/qualification/add"
                            type="button"
                      ...
                      <Table striped bordered hover responsive center>
                        <thead>
                          <tr className="">
                            ...
                            {visible.col2 && (
                              <th className="tax-name">Qualification Name </th>
                            )}
                            {/* {visible.col4 && (
                              <th className="tax-name">Currency </th>
                            )}
                            {visible.col5 && (
                              <th className="tax-name">Country Code </th>
                            )}
                            {visible.col7 && (
                          ...
                          {data?.data?.data?.map((d, index) => {
                            ...
                            return (
                              <tr className="" key={index}>
                                ...
                                {/* {visible.col4 && <td>{d?.currency}</td>}
                                {visible.col5 && <td>{d?.country_code}</td>}
                                {visible.col7 && <td>{d?.image && (
                                  <img
                                    src={IMG_URL + d?.image}
                                    alt="Image"
