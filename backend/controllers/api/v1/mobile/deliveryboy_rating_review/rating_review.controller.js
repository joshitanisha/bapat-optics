...
class RatingReviewsController {
  async create(req, res) {
    ...
    try {
      ...
      const deliveryboy = await Product_Order.findOne({
        where: {
          id: req.body?.order_id,
        },
      });

      data.delivery_boy_id = deliveryboy.delivery_boy_id;
      ...
      // const exists = await CheckExits(Deliveryboy_Rating, { name: data?.name }, t);
      ...
      const newItem = await CreateNew(Deliveryboy_Rating, data, t);
  ...
  async getProductRatings(req, res) {
    const t = await sequelize.transaction();
    try {
      const data = await Deliveryboy_Rating.findAll({
        required: true,
        include: [
          {
            model: Users,
