  ...
  async AddToCart(req, res) {
    ...
    try {
      ...
      const data = {
        ...
        coupon_status: req.body.coupon_status || 0,
      };

      // const existsCart = await CheckExits(Cart, { user_id: data?.user_id }, t);
      // if (existsCart && existsCart?.store_id !== product?.store_id) {
      //   await t.rollback();
      ...
      if (exists) {
        ...
        return Base.sendError(
          ...
          "Product already cart by you"
        );
      }

      const newItem = await CreateNew(Cart, data, t);

      const totalcartItems = await Cart.findAll({
      ...
      const totalCartValue = totalcartItems.reduce((total, item) => {

        return (
          total +
          parseFloat(item?.Product_Variant?.price) * Number(item.quantity || 1)
        );
      }, 0);

      // const coupons = await Coupon.findAll({
      //   order: [["required_amount", "ASC"]],
      //   transaction: t,
      // });
      ...
      const nextCoupon = coupons.find(
        (coupon) => Number(coupon.required_amount) > totalCartValue
      );
      ...
      if (nextCoupon) {
        const diff = (
          Number(nextCoupon.required_amount) - totalCartValue
        ).toFixed(2);

        const message = `Add items worth ₹${diff} more to be eligible for coupon ${nextCoupon.code}.`;
        ...
        const user = await CheckExits(Users, { id: req.user.user_id }, t);
