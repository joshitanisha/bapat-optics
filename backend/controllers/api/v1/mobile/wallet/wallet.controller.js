const { UpdateDataDemo } = require("../../../../../helper/common/function");
// const { CheckExits, UpdateData } = require("../../../../../helper/common/utils/dbUtils");
...
class SubscriptionController {
  ...
  async findAll(req, res) {
    try {
      ...
      if (type === "Referral" || type === "Purchase") {
        whereCondition.type = type;
      }

      const { count, rows: walletHistory } =
        await Wallet_History.findAndCountAll({
          include: [{ model: Transaction_Type }, { model: Product_Order }],
  ...
  async transaction(req, res) {
    ...
    try {
      ...
      if (wallet) {
        const updateWallet = await UpdateDataDemo(
          Wallet,
          { amount: totalAmount },
          { user_id: req?.user?.user_id },
          t
        ...
        const updateWallet = await CreateNew(
          Wallet,
          { user_id: req.user.user_id, amount: amount },
          t
        );
      }
