...
class ProductOrderController {
  ...
  async findAll(req, res) {
    try {
      ...
      const where = { payment_status: payment_status };
      ...
      const include = [
        { model: Users },
        {
          model: Refund_Order_Details,
          include: [
            {
              model: Product_Order_Detail,
              where:ProductDetails,
              // where: {
              //   [Op.or]: [{ status: false }, { return_status: false }],
              // },
              include: [
                {
                  model: Product,
      ...
      const { count, rows: data } = await RefundOrders.findAndCountAll({
        include: include,
        where: where,
