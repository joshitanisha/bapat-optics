...
class ProductController {
  ...
  async findBarModProduct(req, res) {
    const t = await sequelize.transaction();
    try {
      const { barcode_no, model, name, p_category_id, bo_code } = req.query;
        ...
        const options = [
          {
            ...
            // where: { barcode_no: barcode_no, stock_status_id: 1 },
          },
        ];
        ...
        const options = [
          {
            ...
            as: "Stock",
            where: { model: model },
            // where: { model: model, stock_status_id: 1 },
          },
        ];
  ...
  async findBoProduct(req, res) {
    const t = await sequelize.transaction();
    try {
      const { bo_code, lens_name } = req.query;
      ...
      if (lens_name) {
        ...
        // data = await CheckExits(Product, { p_category_id: 1 }, t, options);
      }
  ...
  async registerUser(req, res) {
    const t = await sequelize.transaction();
    try {
      const { name, email, contact_no, age, date_of_birth } = req.body;
  ...
  async CalculationOrder(req, res) {
    ...
    try {
      ...
      for (const item of cartListItems) {
        ...
        const selling_price = parseFloat(productData?.price || 0);
