...
const { Career, Career_Resume, sequelize, Shift, JobType } = require("../../../../../models/index");
...
class CareerController {
    async findAll(req, res) {
        try {
            ...
            const searchJobType = req.query.searchJobType?.trim() || "";
            const searchCompany = req.query.searchCompany?.trim() || "";
            const searchShiftType = req.query.searchShiftType?.trim() || "";
            ...
            const options = {
                ...
                include: [{
                    ...
                    searchCompany ? { company_name: { [Op.like]: `%${searchCompany}%` } } : {},
                        whereClause,

                    ],
                    include: [{
                        model: JobType,
                        where: searchJobType ? { name: { [Op.like]: `%${searchJobType}%` } } : {},
                    },
                    {
                        model: Shift,
                        where: searchShiftType ? { name: { [Op.like]: `%${searchShiftType}%` } } : {},
                    }]
                }],
                order: [['createdAt', 'DESC']],
            }
            await Paginate(Career_Resume, options, req, res, Op);
        } catch (error) {
    ...
    async delete(req, res) {
        ...
        try {
            ...
            const exists = await CheckExits(Career_Resume, { id }, t);
            if (!exists) {
                await t.rollback();
                return Base.sendError(res, HTTPS.NOT_FOUND, "Career_Resume not found");
            }

            await Career_Resume.destroy({ where: { id }, transaction: t });
            await t.commit();

            return Base.sendResponse(res, HTTPS.OK, {}, "Career Resume deleted successfully");
        } catch (error) {
            await t.rollback();
            console.error("Error deleting Career_Resume:", error);
            return Base.sendError(res, HTTPS.INTERNAL_SERVER_ERROR, error);
        }
    }
