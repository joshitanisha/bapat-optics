...
const COLUMN_INDEX_MAP_EYE = {
  ...
  12: "SHAPE",
  13: "SIZE",
  14: "TOTAL MEASUREMENTS",
  15: "DESCRIPTION",
  16: "QUANTITY",
  17: "WLP", 
  18: "DISCOUNT %",
  19: "DISCOUNT AMOUNT",
  20: "BASE PURCHASE PRICE",
  21: "GST Percentage",
  22: "GST AMOUNT",
  23: "TOTAL PURCHASE PRICE",
  24: "HSN CODE",
  25: "SUPPLIER",
  26: "GST NUMBER",
  27: "MRP"
...
const CONTACT_LENS_COLUMN_INDEX_MAP = {
  0: "NO",                  // Replaces "SRNO" to match file exactly
  1: "DATE",
  2: "INVOICE NUMBER",
  3: "ORDER NUMBER",
  4: "CATEGORY",
  ...
  21: "DISCOUNT %",
  22: "DISCOUNT AMOUNT",
  23: "BASE PURCHASE PRICE",
  24: "GST %",              
  25: "GST AMOUNT",
  26: "TOTAL PURCHASE PRICE",
  27: "SUPPLIER NAME",
  28: "GST NUMBER",
  29: "HSN CODE",
  30: "MRP"
...
class ProductController {
  ...
  async BulkUpload(req, res) {
    ...
    try {
      ...
      const rows1 = xlsx.utils.sheet_to_json(sheet, { defval: "" });

      // console.log(`Total Excel Rows Parsed: ${rows1.length}`);

      // Filter out rows where every column value is empty or whitespace
      const rows = rows1.filter(row =>
      ...
      for (let r of rows) {
        ...
        if (categoryCheck?.id == IDS.Category.Sunglasses) {
          ...
          if (r?.[6]) {
            brandCheck = await Brand.findOne({ where: { name: r[6] } });
            if (!brandCheck) {
          ...
          if (r?.[8]) {
            materialCheck = await Material.findOne({ where: { name: r[8] } });
            if (!materialCheck) {
          ...
          if (r?.[9]) {
            ...
            if (!frameTypeCheck) {
              frameTypeCheck = await Frame_Type.create({ name: r[9] });
            }
          }

          // GENDER
          if (r?.[10]) {
            genderCheck = await Gender.findOne({ where: { name: r[10] } });
            if (!genderCheck) {
              genderCheck = await Gender.create({ name: r[10] });
            }
          }

          // FRAME COLOR
          if (r?.[11]) {
            colorCheck = await Colour.findOne({ where: { name: r[11] } });
            if (!colorCheck) {
              colorCheck = await Colour.create({ name: r[11] });
          ...
          // LENS COLOR
          if (r?.[12]) {
            lensColorCheck = await Colour.findOne({ where: { name: r[12] } });
            if (!lensColorCheck) {
              lensColorCheck = await Colour.create({ name: r[12] });
          ...
          // SHAPE
          if (r?.[13]) {
            shapeCheck = await Shape.findOne({ where: { name: r[13] } });
            if (!shapeCheck) {
              shapeCheck = await Shape.create({ name: r[13] });
            }
          }
          ...
          name = [
            r?.[6], //r?.[3], // r?.[2],
            r?.[13], //r?.[10], // shape color
            r?.[11],  //r?.[8], // frame color
            r?.[7], // r?.[4], // r?.[3], // model no
          ]
            .filter(Boolean)
            .join(" ");
          ...
          modelNo = r?.[7] || null; // r?.[4] || null; //r?.[3] || null;
          barcode = r?.[5] || null; // r?.[2] || null; //r?.[4] || null;

          WLP = parseFloat(r?.[18]) || 0.0;   // parseFloat(r?.[14]) || 0.0;
          // console.log("WLP", WLP);

          WLPDiscount = parseFloat(r?.[19]) || 0.0;  /// parseFloat(r?.[15]) || 0.0;
          // console.log("WLPDiscount", WLPDiscount);

          WLPDiscountAmount = parseFloat(r?.[20]) || 0.0;  // parseFloat(r?.[16]) || 0.0;
          // console.log("WWLPDiscountAmountLP", WLPDiscountAmount);

          base_price = parseFloat(r?.[21]) || 0.0;  // parseFloat(r?.[17]) || 0.0;  //parseFloat(r?.[13]) || 0.0;
          // console.log("base_price", base_price);

          gst = parseFloat(r?.[22]) || 0.0; // parseFloat(r?.[18]) || 0.0; //parseFloat(r?.[14]) || 0.0;
          // console.log("GST", gst);

          gst_price = parseFloat(r?.[23]) || 0.0;  // parseFloat(r?.[19]) || 0.0; //parseFloat(r?.[15]) || 0.0;
          // console.log("gst_price", gst_price);

          cost_price = parseFloat(r?.[24]) || 0.0;  // parseFloat(r?.[20]) || 0.0;  //parseFloat(r?.[18]) || 0.0; // sunglasses

          const parsedMrp = parseFloat(r?.[28]) || 0;  // parseFloat(r?.[24]) || 0; //parseFloat(r?.[14]) || 0;
          let baseAmount = 0;
          let taxAmount = 0;
          ...
          currentProduct = {
            name,
            description: (r?.[16] || "").toString(), // (r?.[13] || "").toString(), //(r?.[15] || "").toString(),
            image: `/public/uploads/Product/${barcode}-1.jpg`,
            price: sellingPrice || 0.0,
            mrp: parseFloat(r?.[28]) || 0.0,  // parseFloat(r?.[24]) || 0.0, //parseFloat(r?.[14]) || 0.0,
            ...
            size: r?.[14] || null, //r?.[11] || null,
            total_measurements: r?.[15] || null, // r?.[12] || null,
            model_no: modelNo,

            approval_status_id: IDS.ApprovalStatus.Approved,
            available_stock: 0,
            barcode_status: 1,
            hsn_code: r?.[25] ?? "", // r?.[21] ?? "", //r?.[13] ?? "",
            customer_view: false,
          };
        ...
        if (categoryCheck?.id == IDS.Category.Eyeglasses || categoryCheck?.id == IDS.Category.SPECTACLE_FRAMES) {
          // missingFields = getMissingFields(r, COLUMN_INDEX_MAP_EYE, ["WLP"]);
          supplier = await Supplier.findOne({
            where: {
              gst_no: r[26], //r[22], //r[19], //r[16],
            },
          });
          if (!supplier) {
            supplier = await Supplier.create({
              name: r[25], // r[21],  //r[18],  // r[15],   // Supplier Name 
              gst_no: r[26], //r[22], // r[19], // r[16],  // GST NO
            });
          }
          ...
          name = [r?.[6], r?.[12], r?.[11], r?.[7]].filter(Boolean).join(" ");  // New changes final
          // name = [r?.[3], r?.[9], r?.[8], r?.[4]].filter(Boolean).join(" ");  // New changes
          // name = [r?.[2], r?.[9], r?.[8], r?.[3]].filter(Boolean).join(" ");
          // Brand Name. Shape, Color, Model No
          ...
          WLP = parseFloat(r?.[17]) || 0.0;  //parseFloat(r?.[13]) || 0.0;
          // console.log("WLP", WLP);

          WLPDiscount = parseFloat(r?.[18]) || 0.0; //parseFloat(r?.[14]) || 0.0;
          // console.log("WLPDiscount", WLPDiscount);

          WLPDiscountAmount = parseFloat(r?.[19]) || 0.0; //parseFloat(r?.[15]) || 0.0;
          // console.log("WWLPDiscountAmountLP", WLPDiscountAmount);

          base_price = parseFloat(r?.[20]) || 0.0; //parseFloat(r?.[16]) || 0.0;  //parseFloat(r?.[13]) || 0.0;
          // console.log("base_price", base_price);

          gst = parseFloat(r?.[21]) || 0.0;  //parseFloat(r?.[17]) || 0.0; //parseFloat(r?.[14]) || 0.0;
          // console.log("GST", gst);

          gst_price = parseFloat(r?.[22]) || 0.0; // parseFloat(r?.[18]) || 0.0; //parseFloat(r?.[15]) || 0.0;
          // console.log("gst_price", gst_price);

          cost_price = parseFloat(r?.[23]) || 0.0;  //parseFloat(r?.[19]) || 0.0; //parseFloat(r?.[16]) || 0.0; // parseFloat(r?.[17]) || 0.0; // Total Purchase Price  // EyeGlasses
          let category = await p_category.findOne({
            where: {
              id: IDS.Category.Eyeglasses,
            },
          });
          const parsedMrp = parseFloat(r?.[27]) || 0; // parseFloat(r?.[23]) || 0;  //parseFloat(r?.[20]) || 0;  //parseFloat(r?.[13]) || 0;
          let baseAmount = 0;
          let taxAmount = 0;
          let discountAmount = 0;
          let sellingPrice = 0;
          ...
          currentProduct = {
            ...
            description: (r?.[15] || "").toString(),  // //(r?.[14] || "").toString(),
            price: sellingPrice || 0.0,
            mrp: parseFloat(r?.[27]) || 0.0,  //parseFloat(r?.[20]) || 0.0,  //parseFloat(r?.[13]) || 0.0,
            discount: parsedDiscount || 0.0,
            discount_amount: discountAmount || 0.0,
            tax_percentage: parsedTax || 0.0,
            tax_amount: taxAmount || 0.0,
            ...
            frame_type_id: frameTypeCheck?.id || null,
            material_id: materialCheck?.id || null,
            size: r?.[13] || null, //r?.[10] || null,
            total_measurements: r?.[14] || null,  //r?.[11] || null,
            model_no: modelNo,
            approval_status_id: IDS.ApprovalStatus.Approved,
            available_stock: 0,
            barcode_status: 1,
            hsn_code: r?.[24] ?? "", // r?.[17] ?? "", // r?.[12] ?? "",
            customer_view: false,
          };
        ...
        if (categoryCheck?.id == IDS.Category.ContactLens) {
          ...
          name = [r?.[8], r?.[9], r?.[5]].filter(Boolean).join(" ");
          // name = [r?.[4], r?.[5], r?.[1]].filter(Boolean).join(" ");
          // Company NAme, Product, BO CODE
          bocode = r?.[1];
          ...
          const parsedMrp = parseFloat(r?.[30]) || 0; //parseFloat(r?.[14]) || 0;
          let baseAmount = 0;
          let taxAmount = 0;
          let discountAmount = 0;
          let sellingPrice = 0;
          ...
          price = sellingPrice || 0.0;
          cost_price = parseFloat(r?.[26]) || 0.0; // parseFloat(r?.[17]) || 0.0; //ContactLens
          currentProduct = {
            name: name,
            image: (`/public/uploads/Product/${bocode}-1.jpg` || "").toString(),
            description: (r?.[19] || "").toString(), //(r?.[18] || "").toString(),
            size: (r?.[15] || "").toString(), // (r?.[11] || "").toString(),
            price: sellingPrice,
            mrp: parseFloat(r?.[30]) || 0.0, // parseFloat(r?.[14]) || 0.0,
            discount: parsedDiscount || 0.0,
            ...
            brand_id: brandCheck?.id || null,
            material_id: materialCheck?.id || null,
            bo_code: r?.[5] || null,  // r?.[1] || null,
            modality: r?.[6] || null, // r?.[2] || null,
            dk_t: r?.[14] || null, //  r?.[10] || null,
            water_content: r?.[11] || null, // r?.[7] || null,
            diameter: r?.[13] || null, // r?.[9] || null,
            base_curve: r?.[12] || null, // r?.[8] || null,
            approval_status_id: IDS.ApprovalStatus.Approved,
            available_stock: 0,
            barcode_status: 0,
            manufacturer: r?.[9], // r?.[5],
            hsn_code: r?.[29] ?? "", // r?.[19] ?? "",
            customer_view: false,
          };

          WLP = parseFloat(r?.[20]) || 0.0; // parseFloat(r?.[14]) || 0.0;
          // console.log("WLP", WLP);

          WLPDiscount = parseFloat(r?.[21]) || 0.0; // parseFloat(r?.[15]) || 0.0;
          // console.log("WLPDiscount", WLPDiscount);

          WLPDiscountAmount = parseFloat(r?.[22]) || 0.0; // parseFloat(r?.[16]) || 0.0;
          // console.log("WWLPDiscountAmountLP", WLPDiscountAmount);

          base_price = parseFloat(r?.[23]) || 0.0;  // parseFloat(r?.[17]) || 0.0;  //parseFloat(r?.[13]) || 0.0;
          // console.log("base_price", base_price);

          gst = parseFloat(r?.[24]) || 0.0; // parseFloat(r?.[18]) || 0.0; //parseFloat(r?.[14]) || 0.0;
          // console.log("GST", gst);

          gst_price = parseFloat(r?.[25]) || 0.0;  // parseFloat(r?.[19]) || 0.0; //parseFloat(r?.[15]) || 0.0;
          // console.log("gst_price", gst_price);
        }
        ...
        if (categoryCheck?.id == IDS.Category.Lenses) {
          ...
          name = [r?.[6], r?.[7], r?.[5]].filter(Boolean).join(" ");
          // name = [r?.[0], r?.[8], r?.[2]].filter(Boolean).join(" ");
          // Brand Name, Product Name, BO Code

          const parsedMrp = parseFloat(r?.[27]) || 0; // parseFloat(r?.[10]) || 0;
          let baseAmount = 0;
          let taxAmount = 0;
          let discountAmount = 0;
          let sellingPrice = 0;
          ...
          bocode = r?.[5]; // r?.[2];
          price = sellingPrice || 0.0;
          cost_price = parseFloat(r?.[23]) || 0.0; // parseFloat(r?.[15]) || 0.0; //Lenses

          currentProduct = {
            name: name,
            image: (`/public/uploads/Product/${bocode}-1.jpg` || "").toString(),
            description: (r?.[15] || "").toString(), // (r?.[16] || "").toString(),
            price: sellingPrice || 0.0,
            mrp: parsedMrp || 0.0,
            ...
            bo_code: r?.[5] || null, // r?.[2] || null,
            // size: r?.[17] || null,
            index: r?.[11] || null, // r?.[7] || null,
          ...
          WLP = parseFloat(r?.[17]) || 0.0; // parseFloat(r?.[14]) || 0.0;
          // console.log("WLP", WLP);

          WLPDiscount = parseFloat(r?.[18]) || 0.0; // parseFloat(r?.[15]) || 0.0;
          // console.log("WLPDiscount", WLPDiscount);

          WLPDiscountAmount = parseFloat(r?.[19]) || 0.0; // parseFloat(r?.[16]) || 0.0;
          // console.log("WWLPDiscountAmountLP", WLPDiscountAmount);

          base_price = parseFloat(r?.[20]) || 0.0;  // parseFloat(r?.[17]) || 0.0;  //parseFloat(r?.[13]) || 0.0;
          // console.log("base_price", base_price);

          gst = parseFloat(r?.[21]) || 0.0; // parseFloat(r?.[18]) || 0.0; //parseFloat(r?.[14]) || 0.0;
          // console.log("GST", gst);

          gst_price = parseFloat(r?.[22]) || 0.0;  // parseFloat(r?.[19]) || 0.0; //parseFloat(r?.[15]) || 0.0;
          // console.log("gst_price", gst_price);
        }
        ...
        if (exists) {
          ...
          if (
            ...
            categoryCheck?.id == IDS.Category.SPECTACLE_FRAMES
          ) {
        ...
        if (name && !exists) {
          ...
          for (let i = 1; i <= 2; i++) {
            await CreateNew(
              Product_Images,
              {
                ...
                image: `/public/uploads/Product/${barcode || bocode
                  }-${i}.jpg`.toString(),
              },
              t,
            );
          ...
          if (
            ...
            if (bocode && exists) {
              quantity = categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] || 1 : r?.[16] || 1;

              const quantityloop = Number(quantity || 1);
              ...
              const Receivingdata = await CreateNew(
                Receiving,
                {
                  ...
                  wlp: WLP,
                  wlpdiscount: WLPDiscount,
                  wlpdiscountamount: WLPDiscountAmount,
                  base_price: WLP - WLPDiscountAmount,
                  total_price: cost_price || price,

                  receiving_date: convertExcelToDateTime(r?.[1]),
                },
                t,
              );
              await CreateNew(
              ...
              const receivingProduct = await CreateNew(
                Receiving_Product,
                {
                  ...
                  total_price: cost_price || price, // Amol
                  price: base_price,  //cost_price || price, // Amol
                  description: "Stock add by bulk upload ",

                  // total_price: cost_price || price,  // Amol
                  // price: cost_price || price, // Amol
                  // description: "Stock add by bulk upload ",
                },
                t,
              );
              ...
              const PurchaseHistory = await CreateNew(
                Purchase_History,
                {
                  p_o_id: PurchaseOrder.id, //req.body.id,
                  p_o_s_id: IDS.PurchaseOrderStatus.Ordered,
                  comment: "Order Converted To Receiving",
                  user_id: req.user.user_id,
                  supplier_id: supplier?.id,
        ...
        if (barcode && !existsBarcode) {
          ...
          const PurchaseOrder = await CreateNew(
            Purchase_Order,
            {
              ...
              user_id: req.user.user_id,
              supplier_id: supplier?.id,
              total_quantity: categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] || 1 : r?.[16] || 1,  // 1,
            },
            t,
          );
          ...
          const PurchaseOrderProduct = await CreateNew(
            Purchase_Order_Product,
            {
              ...
              p_o_id: PurchaseOrder.id,
              product_id: exists.id,
              quantity: categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] || 1 : r?.[16] || 1,  //1,
              description: "Stock add by bulk upload ",
            },
            t,
          );

          // console.log("Receiving Date ", r?.[1]);
          // const excelDate = r?.[1];
          // const jsDate = new Date(Date.UTC(1899, 11, 30) + excelDate * 86400000);
          // // console.log(jsDate.toISOString().split("T")[0]); // YYYY-MM-DD
          // const receiving_date = jsDate.toISOString().split("T")[0];
          const Receivingdata = await CreateNew(
            Receiving,
            {
              p_o_id: PurchaseOrder?.id,
              ...
              user_id: req.user.user_id,
              supplier_id: supplier.id,
              quantity: categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] : r?.[16],  //1,
              invoice_no: r?.[2], //r?.[23],
              order_no: r?.[3], //r?.[24],
              // expiry_date: req.body.expiry_date,
              ...
              receiving_date: convertExcelToDateTime(r?.[1]), //receiving_date, // r?.[1],
            },
            t,
          );
          ...
          const productstock = await CreateNew(
            Product_Stock,
            {
              ...
              product_id: exists?.id,
              // variant_id: variant.variant_id,
              general_stock: categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] || 1 : r?.[16] || 1,  //1,
              // selling_price: variant.selling_price,
              // subscription_stock: variant.subscription_stock,
              user_id: req.user.user_id,
              receiving_id: Receivingdata.id,
          ...
          await CreateNew(
            ...
            {
              stock_id: stock?.id,
              name: "stock added By Bulk Upload",
            },
            t,
          );
          ...
          await CreateNew(
            Purchase_History,
            {
              ...
              user_id: req.user.user_id,
              supplier_id: supplier?.id,
              total_quantity: categoryCheck?.id === IDS.Category.Sunglasses ? r?.[17] || 1 : r?.[16] || 1,
            },
            t,
          );
          // }
  ...
  async VtoStatus(req, res) {
    ...
    try {
      ...
      await UpdateData(
        Product,
        { vto_enable: result.vto_enable ? false : true },
        { id },
        t,
      );
...
function convertExcelToDateTime(excelValue) {
  const excelDate = excelValue;
  const jsDate = new Date(Date.UTC(1899, 11, 30) + excelDate * 86400000);
  // console.log(jsDate.toISOString().split("T")[0]); // YYYY-MM-DD
  const receiving_date = jsDate.toISOString().split("T")[0];

  return receiving_date || null; // Return null if invalid
}
