  ...
  async findAll(req, res) {
    try {
      ...
      const { count, rows: data } =
        await Subscription_Product_Details.findAndCountAll({
          include: [
            { model: Product, required: true, where: whereClauseProduct },
            ...
            { model: Subscription_Week_Details, required: true },
            {
              model: Subscription_Delivery_Details,
              where,
              include: [
                { model: Time_Slot, required: true, where: timeSlotWhere },
              ],
            },
          ],
          // where: whereClause,
    ...
    try {
      ...
      subscriptionRecords.forEach((data) => {
        const result = {
          ...
          customer_name: data?.Subscription?.User?.name || "-",
